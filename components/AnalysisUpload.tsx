...
import { parseFlukeCSV } from './FlukeCSVParser';
...
export const AnalysisUpload: React.FC<AnalysisUploadProps> = ({ onAnalysisComplete }) => {
    ...
    const processFile = async (file: File) => {
        ...
        try {
            // --- Fluke CSV Auto-Detection ---
            // Read raw text first for CSV files to check if it's a Fluke format
            if (file.name.endsWith('.csv')) {
                const rawText = await file.text();
                const firstLine = rawText.split(/\r?\n/)[0] || '';

                if (firstLine.includes('Internal ProfileID') || firstLine.includes('ProfileID')) {
                    // This is a Fluke meter CSV — use custom parser
                    setProcessingStep(1);
                    const flukeResult = parseFlukeCSV(rawText);

                    if (flukeResult.readings.length === 0) {
                        throw new Error("Fluke file detected but no valid battery readings found.");
                    }

                    setProcessingStep(2);

                    // Convert Fluke readings → CellInput[]
                    const parsedCells: CellInput[] = flukeResult.readings.map(r => ({
                        cellId: r.id,
                        voltage: r.voltage,
                        impedanceOhms: r.resistance > 0 ? r.resistance / 1000 : undefined, // mΩ → Ω
                        temperature: r.temperature || 25,
                        ratedAh: parseInt(flukeResult.metadata.capacity) || nominalCapacity,
                    } as CellInput));

                    setProcessingStep(3);

                    // Auto-set capacity from Fluke metadata
                    const flukeCapacity = parseInt(flukeResult.metadata.capacity) || nominalCapacity;

                    const meta = {
                        siteId: flukeResult.metadata.location || 'Fluke Import',
                        assetId: flukeResult.metadata.deviceName || file.name.split('.')[0],
                        operator: 'Fluke BT500',
                        date: flukeResult.metadata.timeCreated || new Date().toISOString(),
                        chemistryId: chemistryType,
                        nominalCapacityAh: flukeCapacity,
                    ...
                    return; // Done — skip standard path
                }
            }
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                {[
                    ...
                    <button
                        ...
                        className={`px-3 py-2 rounded border text-left transition-all ${chemistryType === chem.id
            ...
            {entryMode === 'upload' ? (
                <>
                    <div
                        ...
                        onDrop={handleDrop}
                        className={`border-2 border-dashed rounded-xl p-12 text-center transition-all duration-200 ${isDragOver
